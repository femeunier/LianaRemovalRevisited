...
E <- raster::extract(R <- raster("./unzipdir/E.nc"),cbind(-79.85,9.15),method = "bilinear")
...
for (model in Names){
  ...
  for (model.form in seq(1,length(cfixed.effect.2.test))){
    ...
    op.file <- file.path(".","outputs",paste0("Fit.Biomass2.",cname,".RDS"))
