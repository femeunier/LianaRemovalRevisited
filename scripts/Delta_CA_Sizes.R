...
all.df <- bind_rows(readRDS("./outputs/All.CA.data.RDS") %>%
                      mutate(sp = str_squish(sp)) %>%
                      filter(dbh >= 10),
                    readRDS("./outputs/All.CA.data.RDS") %>%
                      mutate(sp = str_squish(sp)) %>%
                      filter(dbh >= 10) %>%
                      mutate(site = "Total"))
...
Ndf <- all.df %>%
  group_by(site) %>%
  summarise(Nlarge = n(),.groups = "keep")
...
site.loc <- readRDS("./outputs/site.loc.RDS") %>%
  ...
  dplyr::select(-Nlarge) %>%
  left_join(Ndf, by = "site")

sites <- unique(all.df$site)
sites.fac <- c(sites[order(sites[sites != "Total"])],"Total")

# Model predictions
Model.predictions <- bind_rows(readRDS("./outputs/Model.predictions.CA.25.RDS") %>%
                                 mutate(target = 25),
  ...
  summarise(m = round(100*mean(diff_CA/no,na.rm = TRUE),digits = 1),
            CIlow = round(100*quantile(diff_CA/no,0.11/2,na.rm = TRUE),digits = 1),
            CIhigh = round(100*quantile(diff_CA/no,1-0.11/2,na.rm = TRUE),digits = 1),
            .groups = "keep")
...
A <- ggplot(data = Nvseffect.wide %>%
  ...
  geom_errorbarh(aes(xmin = CIlow_50, xmax = CIhigh_50),height = 0) +
  geom_errorbar(aes(ymin = CIlow_25, ymax = CIhigh_25),width = 0) +
...
B <- ggplot(data = Nvseffect.wide %>%
  ...
  geom_point(aes(size = Nlarge)) +
  geom_errorbarh(aes(xmin = CIlow_50, xmax = CIhigh_50),height = 0) +
  geom_errorbar(aes(ymin = CIlow_100, ymax = CIhigh_100),width = 0) +
  # stat_smooth(method = "lm", color = "black") +
...
C <- ggplot(data = Nvseffect.wide %>%
  ...
  geom_point(aes(size = Nlarge)) +
  geom_errorbarh(aes(xmin = CIlow_50, xmax = CIhigh_50),height = 0) +
  geom_errorbar(aes(ymin = CIlow_150, ymax = CIhigh_150),width = 0) +
  # stat_smooth(method = "lm", color = "black") +
