...
AGB_func <- function(x, wd){
  H <- 51.38*(1-exp(-0.01322*(x)^0.6465))
...
#    CENSUS <- t(diff(t(cbind(x$Date_1, x$Date_3, x$Date_5, x$Date_7, x$Date_9, x$Date_11, x$Date_13, x$Date_15, x$Date_17, x$Date_19, x$Date_21))))
    AGB <- apply(cbind(DIAM[,12:22]), 2, AGB_func, wd=DIAM$WD)
...
matplot(difft_median[12:17,c(1,3,5)],type = "l",lty = 1)
abline(h = 0)
...
# saveRDS(plottot_epl,file = "./outputs/plottot_epl_ref.RDS")
saveRDS(plottot_ept,file = "./outputs/plottot_ept_ref.RDS")
# saveRDS(plottot_eptl,file = "./outputs/plottot_eptl_ref.RDS")
