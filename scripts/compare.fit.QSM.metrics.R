...
cols2test <- c("crown_area_conv","crown_length","crown_vol",
               "branch_area","Nbranches","branch_len",
...
ggplot(mapping = aes(x = 100*(value-no)/no, y = variable,
...
ggplot(data = data2plot %>%
         filter(variable %in% c("tree_biomass","branch_biomass")) %>%
         mutate(variable = factor(variable,
                                  levels = c("tree_biomass","branch_biomass"))) %>%
         filter(!is.na(value)),
       aes(x = 100*(value-no)/no,
           fill = liana.cat, color = liana.cat, alpha = signif_rel)) +
  stat_halfeye(color = NA) +
  stat_pointinterval(aes(y = -0.0, alpha = signif_rel2),
                     .width = c(1 - alpha),
                     position = position_dodge(width = 0.02)) +
  geom_vline(xintercept = 0, linetype = 1) +
  scale_y_continuous(limits = c(-0.1,1), breaks = c()) +
...
ggplot() +
  geom_point(data = data.long %>%
               filter(variable %in% c("branch_area","Nbranches","branch_len")) %>%
               mutate(variable = factor(variable,
                                        levels = c("branch_area","branch_len","Nbranches"))),
             aes(x = dbh, y = value, color = liana.cat),
             alpha = 0.2, size = 2) +
  geom_line(data = predictions.long %>%
              filter(variable %in% c("branch_area","Nbranches","branch_len")) %>%
