...
df.all.OP <- data.frame()
count <- overall.count <- 1

for (ibootstrap in seq(1,Nbootstrap)){
  ...
  # ggplot() +
  #
  #   geom_raster(data = df.average %>%
  #                 filter(!is.na(delta_AGB)) %>%
  #               filter((y) <= lat.max, y >= lat.min),
  #             aes(x = x, y = y,fill = delta_AGB),
  #             alpha = 1) +
  #   scale_fill_gradient(low = "darkred",high = "white",na.value = "lightgrey",
  #                        limits = c(-2,0),
  #                        oob=scales::squish) +
  #   geom_sf(data = world,
  #           fill = NA,
  ...
  df.OP <- df.average %>%
    ...
    dplyr::select(lon,lat,delta_bound_av,delta_AGB,AGB.av,area) %>%
    mutate(map = selected.map)
  ...
  df.all.OP <- bind_rows(df.all.OP,
                         df.OP %>%
                           mutate(ID = ibootstrap))

  if ((count %% 100) == 0){
    print(paste0("Saving ",overall.count))
    saveRDS(df.all.OP,
            file = paste0("./outputs/Bootstrap_spatial.",overall.count,".RDS"))
    count <- 1
