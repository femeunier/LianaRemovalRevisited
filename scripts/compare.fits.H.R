...
model.forms <- c("a","none","all","b","ak","bk","k","ab")
...
temp <- bind_rows((lapply(1:length(years),
                          function(iyear){
                            ...
                            pep <- melt(posterior_epred(cmodel,
                              ...
                              rename(rep = Var1,
                                     id = Var2) %>%
                              mutate(h.pred = ccoef.null*exp(value)) %>%
                              group_by(id) %>%
...
ThreeD.height <- readRDS("./outputs/plot3d.height.RDS")
...
ggplot() +
  ...
  geom_point(data = all.df %>%
               filter(year %in% years,
                      Tag %in% c(ThreeD.height[["Tag"]])),
             aes(x = dbh, y = h, col = liana.cat), alpha = 0.8, size = 2) +

  geom_point(data = all.df %>%
               filter(year %in% years),
             aes(x = dbh, y = h, col = liana.cat), alpha = 0.2, size = 2) +

  scale_color_manual(values = c("no" = "darkgreen",
                                "low" = "orange",
                                "high"= "darkred")) +
  geom_line(data = temp ,
            aes(x = dbh, y = h.pred.m, color = liana.cat)) +
...
saveRDS(temp2,
        paste0("./outputs/BCI.H.TLS.",DBH2test,".RDS"))
