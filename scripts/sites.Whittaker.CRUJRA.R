...
for (isite in seq(1,nrow(site.loc))){
  ...
  r.tavg <- stack("./outputs/CRUJRA/tmp.tif")
  ...
  r.prec <- stack("./outputs/CRUJRA/pre.tif")
  prec.avg <- as.numeric(as.vector(raster::extract(r.prec, xy)))*4*days
  e.avg <- days*3.33
  ...
  r.srad <- stack("./outputs/CRUJRA/dswrf.tif")
  ...
  # VPD
  r.vapr <- stack("./outputs/CRUJRA/VPD.tif")
