...
all.df.title <- all.df %>%
  group_by(site.group) %>%
  mutate(site.N = paste0(site.group,", N = ", length(site.group)," (",length(site.group[which(liana.cat == "no")]), "-",
...
df.residuals %>%
  ...
  summarise(med = median((value),na.rm = TRUE),
            N = n(),
            RMSE = sqrt(1/(n())*(sum(value**2,na.rm = TRUE))),
            MSA = 100*exp(median(abs(value),na.rm = TRUE) - 1))
...
Main.OP <- bind_rows(readRDS("./outputs/Main.OP.25.sitegroups.RDS") %>%
                       mutate(target = 25),
                     readRDS("./outputs/Main.OP.50.sitegroups.RDS") %>%
                       mutate(target = 50),
                     readRDS("./outputs/Main.OP.100.sitegroups.RDS") %>%
                       mutate(target = 100),
                     readRDS("./outputs/Main.OP.150.sitegroups.RDS") %>%
                       mutate(target = 150))

ggplot(data = Main.OP,
       aes(x = diff_h/no*100,
           y = as.factor(site.group),
           color = liana.cat,
           fill = liana.cat,
           alpha = 0.7)) +
...
diff.H <- Main.OP %>%
  group_by(site.group,target,liana.cat) %>%
  summarise(m = median(diff_h/no*100,na.rm = TRUE),
            low = quantile(diff_h/no*100,alpha/2,na.rm = TRUE),
            high = quantile(diff_h/no*100,1-alpha/2,na.rm = TRUE),
